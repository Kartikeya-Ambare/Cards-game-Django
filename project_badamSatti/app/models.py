...
class Game(models.Model):
    ...
    def _get_valid_moves(self, player_hand, desk_cards):
        """
        Determines which cards in a player's hand are valid to play.
        Returns a list of card numbers.
        """
        valid_moves = []
        
        # ▼▼▼ START OF CHANGE ▼▼▼
        # Check if the desk is completely empty (i.e., no cards played yet)
        is_desk_empty = all(not cards for cards in desk_cards.values())

        if is_desk_empty:
            # If the desk is empty, the ONLY valid first move is the 7 of Hearts (card_num=7)
            for card_num, card_name in player_hand:
                if card_num == 7: # 7 is the number for 7 of Hearts
        ...
        for card_num, card_name in player_hand:
            ...
            # 1. A 7 can always be played to start a new suit pile
            if rank == '7':
                valid_moves.append(card_num)
                continue
            
            # 2. Other cards can only be played if the 7 of that suit is on the desk
            if ('7' + suit) not in desk_suit_cards.get(suit, []):
                continue

            # 3. Check for adjacent cards on the desk
    ...
    def update_game_state_after_move(self, card_num, player_num):
        ...
        if not card_to_play:
            print(f"ERROR: Card {card_num} not found in player {player_num}'s hand.")
            return False
    ...
    def pass_turn(self, player_num):
        valid_moves = self.get_valid_moves_for_player(player_num)
        if valid_moves:
            return False, "You have valid moves and cannot pass your turn."
